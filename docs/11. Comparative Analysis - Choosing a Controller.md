...
**State-space control** is the right choice when the operating range stays close to equilibrium, the model is reasonably well-known, and multiple states need to be coordinated through full feedback. Its tuning effort is comparable to PID (designer chooses pole locations or LQR weights), its real-time cost is essentially identical to PID (a matrix-vector product), and its scaling to higher dimensions is dramatically better than any of the nonlinear methods (§11.3).
