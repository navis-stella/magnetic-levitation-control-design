...
- `TimeResponse_Noise_enabled.png` — overlay of air-gap trajectories with a settling-process subplot and a steady-state-region subplot.
- `PerformanceCriteria_Noise_enabled.png` — bar chart comparing overshoot, rise time, settling time, and steady-state error across the five controllers.
- `simulation_results_Noise_enabled_.mat` — raw signals from each run, available for any user-defined post-processing.
